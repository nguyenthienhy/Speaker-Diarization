...
SAVED_MODEL_NAME = '/content/drive/MyDrive/Speaker-Diarization/speaker-diarization/uisrnn/pre_trained/saved_model.uisrnn_benchmark'
...
def main(wav_path, embedding_per_second=1.0, overlap_rate=0.5):
    ...
    import shutil
    audio_dir = "/content/drive/MyDrive/Speaker-Diarization/speaker-diarization/sample"
    audio_name = "ahnss.wav"
    customer_dir = "/content/drive/MyDrive/Speaker-Diarization/speaker-diarization/sample/detected/customer"
    consultant_dir = "/content/drive/MyDrive/Speaker-Diarization/speaker-diarization/sample/detected/consultant"
    shutil.rmtree(customer_dir + '/' + audio_name.replace(".wav", ""))
    shutil.rmtree(consultant_dir + '/' + audio_name.replace(".wav", ""))
    os.mkdir(customer_dir + '/' + audio_name.replace(".wav", ""))
    os.mkdir(consultant_dir + '/' + audio_name.replace(".wav", ""))
    for spk,timeDicts in speakerSlice.items():
        print('========= ' + str(spk) + ' =========')
        if spk == 0:
          for i, timeDict in enumerate(timeDicts):
              s = timeDict['start']
              e = timeDict['stop']
              os.system('ffmpeg -ss ' + str(s / 1000) + ' -t ' 
                + str((e - s) / 1000) + 
                ' -i ' + ' ' + audio_dir + '/' + audio_name + ' ' + 
                audio_dir + '/detected/customer/' 
...
if __name__ == '__main__':
    main(r'sample/ahnss.wav', embedding_per_second=1.2, overlap_rate=0.2)